...
import type { Pet } from "@catalog/pets/domain/models/pet.entity";
...
export class MatchScoringService {
  calcularScore(pet: Pet, questionario: QuestionarioMatch): number {
    let score = 0;
  ...
  private scoreMoradia(pet: Pet, q: QuestionarioMatch): number {
    ...
    if (moradia === "casa_quintal_grande" || moradia === "casa_quintal_pequeno") return 25;
    if (moradia === "apartamento_lazer") return 25;
    return 22;
  }

  private scoreDisponibilidade(pet: Pet, q: QuestionarioMatch): number {
    ...
    const dependente = especie === "cao" && (porte === "grande" || porte === "medio");
  ...
  private scoreCriancas(pet: Pet, q: QuestionarioMatch): number {
    const criancas = q.criancasEmCasa;
    if (criancas === "nao") return 20;
    const temperamento = (pet.temperamento ?? "").toLowerCase();
    const amigavel = ["sociável", "carinhoso", "brincalhão", "tranquilo", "apegado"].some(
      (t) => temperamento.includes(t),
    );
    const instavel = ["medroso", "agressivo", "territorial"].some(
      (t) => temperamento.includes(t),
    );
    if (instavel) return criancas === "bebe" || criancas === "crianca_pequena" ? 2 : 10;
    if (amigavel) return 20;
    return criancas === "bebe" ? 10 : 15;
  }

  private scoreOutrosPets(pet: Pet, q: QuestionarioMatch): number {
  ...
  private scorePerfilCompanheiro(pet: Pet, q: QuestionarioMatch): number {
