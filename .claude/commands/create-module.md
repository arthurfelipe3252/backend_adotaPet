...
1. **Nome do módulo** — ex: `pets`, `adoptions`, `users`
2. **É submódulo?** — Pergunte se será submódulo de algum módulo existente. Se sim, qual módulo pai (ex: `catalog`, `match`, `adoption`, `identity`).
3. **Entidades** — Quais entidades existirão nesse módulo? (pode ser mais de uma, ex: `Pet`, `PetPhoto`)
...
- **Caminho base do módulo**:
  - Submódulo de `catalog` → `src/modules/catalog/<nome-modulo>/`
...
- **Path alias** (para tsconfig.json):
  - Se for submódulo de um módulo que já tem alias (ex: `@catalog/*`), use o alias existente
...
- Entidade de domínio: `PascalCase` (ex: `Pet`)
- DTO: `PascalCase + Dto` (ex: `PetDto`)
- Service: `PascalCase + Service` (ex: `PetService`)
- Controller: `PascalCase + Controller` (ex: `PetsController`) — plural no nome da classe e no `@Controller()`
- Repository interface: `PascalCase + Repository` (ex: `PetRepository`)
- Repository impl: `Drizzle + PascalCase + Repository` (ex: `DrizzlePetRepository`)
- Token DI: `UPPER_SNAKE_CASE` (ex: `PET_REPOSITORY`)
- Variável do schema Drizzle: `camelCase` + `Schema` no plural (ex: `petsSchema`)
- Tabelas no banco: `snake_case` plural (ex: `pets`)
- Colunas no banco: `snake_case` (ex: `created_at`)
- Arquivos: `kebab-case.tipo.ts` (ex: `pet.entity.ts`, `pet.dto.ts`, `pets.controller.ts`)
- Módulo NestJS: `PascalCase + Module` (ex: `PetsModule`)
...
### Se for submódulo (ex: submódulo de `catalog`):

Edite o módulo pai para importar o novo módulo:

```typescript
// catalog.module.ts
import { <Module>Module } from "@catalog/<module>/<module>.module";

@Module({
  imports: [PetsModule, <Module>Module],
})
```
...
2. Edite `tsconfig.json` para adicionar o path alias (se ainda não existir):
...
## Passo 5 — Atualizar o DrizzleService

Após criar os schemas, edite `src/shared/infra/database/drizzle.service.ts` para importar e adicionar os novos schemas ao objeto `schema`.
...
- Instruções para rodar a migration: `npm run db:generate` e depois `npm run db:migrate`
