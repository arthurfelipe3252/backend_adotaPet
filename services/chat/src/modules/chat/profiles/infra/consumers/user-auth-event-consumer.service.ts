...
export class UserAuthEventConsumer implements OnApplicationBootstrap {
  ...
  async onApplicationBootstrap(): Promise<void> {
    await Promise.all([
      this.consume(UserAuthExchangeName.USER_CREATED, UserAuthRoutingKey.USER_CREATED, 'chat.service.user-created'),
      this.consume(UserAuthExchangeName.USER_UPDATED, UserAuthRoutingKey.USER_UPDATED, 'chat.service.user-updated'),
    ]);
  }
