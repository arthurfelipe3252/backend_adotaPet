...
export class QuestionarioMatchService {
  ...
  async calcularMeuMatch(user: JwtUser): Promise<MatchResultResponseDto> {
    return this.calcularInterno(user.sub);
  }
  ...
  async remover(user: JwtUser): Promise<void> {
    const q = await this.questionarioRepo.findByAdotanteId(user.sub);
    if (!q) {
      throw new NotFoundException(
        `Adotante ${user.sub} não possui questionário de match.`,
      );
    }
    await this.questionarioRepo.deleteByAdotanteId(user.sub);
  }
